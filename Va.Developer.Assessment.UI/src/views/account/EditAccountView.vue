  ...
  <div class="border-y-2 border-assessment-secondary-400 flex flex-col">
    <div class="flex w-full justify-between items-center">
      ...
      <button type="button" data-modal-target="add-transaction" data-modal-toggle="add-transaction"
        class="py-3 px-2 bg-assessment-accent-500 rounded-lg text-assessment-accent-50 cursor-pointer hover:bg-assessment-accent-600 focus:ring-4 focus:ring-assessment-accent-300 focus:bg-assessment-accent-700 font-semibold text-sm mx-3 focus:outline-none">Add
        Transaction</button>
    </div>
        ...
        <div class="relative bg-white rounded-lg shadow-sm">
          ...
          <div class="p-4 md:p-5">
            <form class="space-y-4" action="#">
              <div>
                <label for="orderedDate"
                  class="block mb-2 text-sm font-medium text-assessment-secondary-900">Transaction Date</label>
                <input type="date" id="orderedDate"
                  ...
                  placeholder="dd/mm/yyyy" v-model="transactionForm.orderedDate" @input="" />
              </div>
              <div>
                <label for="amount" class="block mb-2 text-sm font-medium text-assessment-secondary-900">Your
...
const get = async () => {
  try {
    const response = await axios.get<Response<Account>>(`https://localhost:7297/api/accounts/${route.params.id}`);
